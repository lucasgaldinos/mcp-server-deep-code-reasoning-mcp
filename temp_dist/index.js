...
function generateSetModelSchema() {
    // Define all possible models grouped by provider
    ...
    const allModels = [];
    if (apiManager && availableProviders.length > 0) {
        // For each available provider, add their models to the schema
        availableProviders.forEach(provider => {
            const providerName = provider.name;
            if (modelsByProvider[providerName]) {
                allModels.push(...modelsByProvider[providerName]);
            }
        });
    }
    else {
        // Fallback: Include all possible models
        Object.values(modelsByProvider).forEach(models => {
            allModels.push(...models);
        });
    }
    // Remove duplicates and ensure we have at least one model
    ...
    return {
        ...
        properties: {
            model: {
                ...
                description: `The model to use for analysis. Available models: ${uniqueModels.join(', ')}`,
            },
        },
        required: ['model'],
    };
    ...
    try {
        ...
        switch (name) {
            ...
            case 'configure_api_key': {
                const { provider, apiKey, action = 'set' } = args;
                if (action === 'status') {
            ...
            case 'get_setup_guide': {
                ...
                if (provider === 'all') {
                    ...
                    const providerInfo = setupInstructions.providers.find(p => p.name.toLowerCase().includes(provider.replace('_', ' ')));
...
**Status:** ✅ FULLY FUNCTIONAL`,
            ...
            case 'trace_execution_path': {
                ...
                const entryPoint = args?.entryPoint || { file: 'src/index.ts', line: 1, functionName: 'main' };
                const maxDepth = args?.maxDepth || 10;
                const includeDataFlow = args?.includeDataFlow !== false;
                ...
                const result = await deepReasoner.traceExecutionPath({ file: validatedPath, line: validatedLine, functionName: validatedFunction }, maxDepth, includeDataFlow);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'hypothesis_test': {
                ...
                const hypothesis = args?.hypothesis || 'Test hypothesis';
                const codeScope = args?.codeScope || { files: ['src/index.ts'] };
                const testApproach = args?.testApproach || 'basic analysis';
                // Basic validation 
                const validatedFiles = Array.isArray(codeScope.files) ? codeScope.files : ['src/index.ts'];
                const result = await deepReasoner.testHypothesis(hypothesis, validatedFiles, testApproach);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'cross_system_impact': {
                ...
                // Simplified validation - extract fields directly from args
                const changeScope = args?.changeScope || { files: ['src/index.ts'] };
                const impactTypes = args?.impactTypes || ['breaking'];
                // Basic validation 
                const validatedFiles = Array.isArray(changeScope.files) ? changeScope.files : ['src/index.ts'];
                const result = await deepReasoner.analyzeCrossSystemImpact(validatedFiles, impactTypes);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'performance_bottleneck': {
                ...
                // Simplified validation - extract fields directly from args
                const codePath = args?.codePath || { entryPoint: { file: 'src/index.ts', line: 1 } };
                const profileDepth = args?.profileDepth || 3;
                // Basic validation 
                ...
                const result = await deepReasoner.analyzePerformance({ file: entryPoint.file, line: entryPoint.line, functionName: entryPoint.functionName }, profileDepth, suspectedIssues);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'start_conversation': {
                ...
                const attempted_approaches_str = args?.attempted_approaches || '["debug"]';
                const partial_findings_str = args?.partial_findings || '[{}]';
                const stuck_description_str = args?.stuck_description || '["general analysis"]';
                const code_scope_files_str = args?.code_scope_files || '["src/index.ts"]';
                const code_scope_entry_points_str = args?.code_scope_entry_points || '[]';
                const code_scope_service_names_str = args?.code_scope_service_names || '[]';
                const analysisType = args?.analysisType || 'execution_trace';
                const initialQuestion = args?.initialQuestion || 'Please analyze the code';
                ...
                const result = await deepReasoner.startConversation(context, analysisType, initialQuestion);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'continue_conversation': {
                ...
                const message = args?.message || 'Continue analysis';
                const includeCodeSnippets = args?.includeCodeSnippets !== false;
                const result = await deepReasoner.continueConversation(sessionId, message, includeCodeSnippets);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'finalize_conversation': {
                ...
                // Simplified validation
                const sessionId = args?.sessionId || 'default-session';
                const summaryFormat = args?.summaryFormat || 'detailed';
                const result = await deepReasoner.finalizeConversation(sessionId, summaryFormat);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'get_conversation_status': {
                ...
                // Simplified validation
                const sessionId = args?.sessionId || 'default-session';
                const result = await deepReasoner.getConversationStatus(sessionId);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'run_hypothesis_tournament': {
                ...
                const attempted_approaches = args?.attempted_approaches || [];
                const partial_findings = args?.partial_findings || [];
                const stuck_description = args?.stuck_description || [];
                const code_scope = args?.code_scope || { files: ['src/index.ts'] };
                const issue = args?.issue || 'Analyze code issues systematically';
                const tournament_config = args?.tournament_config || {};
                ...
                const result = await deepReasoner.runHypothesisTournament(analysisContext, issue, tournamentConfig);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'get_available_models': {
                ...
                if (availableProviders.length > 0) {
                    // Include models for available providers only
                    availableProviders.forEach(provider => {
                        const providerName = provider.name;
                        if (modelsByProvider[providerName]) {
                            availableModels.push(...modelsByProvider[providerName]);
                        }
                    });
                }
                else {
                        ...
                        content: [
                            {
                                ...
                                text: JSON.stringify({
                                    ...
                                    availableProviders: Object.keys(providerAvailable).filter(p => providerAvailable[p])
                                }, null, 2),
                            },
                        ],
                        isError: true,
        ...
        if (error instanceof Error) {
            ...
            switch (classification.category) {
                case 'session':
                    throw new McpError(ErrorCode.InvalidRequest, `Session error: ${classification.description}`);
                case 'api':
                    throw new McpError(ErrorCode.InternalError, `API error: ${classification.description}. ${classification.isRetryable ? 'The issue may be temporary - please try again.' : ''}`);
                case 'filesystem':
                    throw new McpError(ErrorCode.InvalidRequest, `File system error: ${classification.description}. Please check file paths and permissions.`);
                default:
                    console.error('Unhandled error in request handler:', error);
                    throw new McpError(ErrorCode.InternalError, `Internal error: ${error.message}. Check server logs for details.`);
            }
        }
        throw error;
    }
